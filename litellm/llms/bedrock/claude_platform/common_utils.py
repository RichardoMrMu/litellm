...
# Consumed by validate_environment / sign_request; the aws-external-anthropic
# endpoint rejects any of these in the body with "Extra inputs are not permitted"
...
# Valid on Anthropic's first-party Messages API, not yet accepted by the AWS endpoint
...
def resolve_unsupported_override(
    ...
    raw: Final = litellm_params.get("claude_platform_unsupported_params")
    if raw is None:
        return None
